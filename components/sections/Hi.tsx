...
export function Hi() {
  ...
  const [animatedZoom, setAnimatedZoom] = useState(0.2);
  ...
  useEffect(() => {
    setZoom(getZoomSize(window.innerWidth));
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      // Start zoom animation after 0.4s delay
      setTimeout(() => {
  ...
  // Update animated zoom when zoom changes (for window resize)
  useEffect(() => {
    if (isVisible) {
      setAnimatedZoom(zoom);
    }
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-[90vh] overflow-hidden cursor-grab active:cursor-grabbing relative"
    ...
    >
      ...
      {welcomeVisible && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none ">
           ...
           <motion.img
            src="https://avatars.githubusercontent.com/u/175528783?v=4"
            alt="logo"
      ...
      <div
        ...
        style={{
          transform: `translate(${panX * DRAG_MULT}px, ${panY * DRAG_MULT}px) scale(${animatedZoom})`,
